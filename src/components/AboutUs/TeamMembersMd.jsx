...
const TeamMembersMd = () => {
  ...
  return (
    <div className="bg-[#F9FEEC] lg:px-[80px] pt-[64px] pb-[80px] md:block hidden lg:hidden">
      ...
      <div className="overflow-x-auto">
        <div className="flex gap-4">
          {teamMembers.map((member, index) => (
            <div
              key={index}
              className="relative group flex-shrink-0 w-[108px] hover:w-[301px] transition-all duration-300"
              style={{ height: "auto", minHeight: "464px" }}
            >
              {/* Base Image */}
              <img
                src={member.normal}
                alt={`Team member ${index + 1}`}
                className="w-full h-full object-cover grayscale group-hover:hidden transition-all duration-300"
              />

              {/* Hover State Container */}
              <div className="absolute top-0 left-0 w-full h-full opacity-0 group-hover:opacity-100 transition-all duration-300">
                {/* Enlarged Image */}
                <img
                  src={member.enlarged}
                ...
                <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/70 to-transparent">
                  <div className="p-6 text-center">
                    {/* Name */}
                    <h3 className="text-[16px] font-bold font-inter leading-[22.4px] mb-1 text-white">
                      {member.name}
                    </h3>
                    ...
                    {member.country && (
                      <div className="flex items-center justify-center gap-2 mb-4">
                        <span className="text-[12px] font-normal font-inter leading-[17.4px] text-white">
                          {member.country}
                        </span>
                        <img
                          src={flag}
                          alt={`${member.country} flag`}
                          className="w-4 h-3 border bg-white"
                        />
                      </div>
                    )}
